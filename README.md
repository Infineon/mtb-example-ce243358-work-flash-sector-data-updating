# PDL: Work flash sector data updating

This code example demonstrates the use of the Flash PDL driver to perform basic read, write, and erase operations on work flash, and update sector data online. The steps are logged on a UART terminal emulator.

[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce243358-work-flash-sector-data-updating)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-ce243358-work-flash-sector-data-updating](https://github.com/Infineon/mtb-example-CE243358-work-flash-sector-data-updating)" application with the desired name "Work_flash_sector_update" configured for the *KIT_T2G-B-H_LITE* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_T2G_C-2D-6M_LITE --app-id mtb-example-CE243358-work-flash-sector-data-updating --user-app-name code_flash_sector_update --target-dir "C:/mtb_projects"
   ```
...
4. After programming, the application starts automatically. Confirm that "PDL: Work Flash Sector Data Updating" is displayed on the UART terminal.
...
Flash provides non-volatile storage for user firmware, user configuration data, and bulk data storage. This driver allows you to read and write data to flash. It also provides information about the address and characteristics of the flash block(s) on the device. 

During the flash writing process, it is important to not reset the device (including XRES pin, software reset, and watchdog), as this may cause unexpected changes to portions of the flash. Also, the low-voltage detection circuits should be configured to generate an interrupt instead of performing a reset.

In this example, the Flash_PDL driver is configured to read the entire sector data from target sector (sector_0), program that data into a backup sector (sector_1), erase the target sector, then read back the source data from sector_1. The data is processed (some values are modified), and then programmed back into sector_0.
...
**Table 1. Application resources**
 ...
 FLASH      |    Flash            | Flash operates using the flash hardware block 
<br>
...
Document title: *CE243358 – PDL: Work flash sector data updating* 
